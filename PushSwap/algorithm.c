...
static int	rotations(int a, t_stack *b)
{
	...
	while (i < stack_size(b))
	{
		forwards += (i + 1) * (((a >= b->value && (a <= stack_end(b)
			|| stack_end(b) < b->value)) || (a <= b->value && a <=
			stack_end(b) && stack_end(b) < b->value)) && forwards == -1);
...
static void	n_rotations(t_stack **s, int n1, t_stack **h, int n2)
{
	...
	while (h && *h && n2 < 0)
	{
		printf("%s", rotate(h, "rrb\n"));
		n2++;
	}
	while (h && *h && n2 > 0)
	{
		printf("%s", rotate(h, "rb\n"));
		n2--;
	}
}
...
void	sort_stack(t_stack **s)
{
	...
	while (h)
	{
		printf("%s", move_top(&h, s, 0));
		while (h && stack_end(*s) <= (*s)->value && stack_end(*s) >= h->value)
			printf("%s", rotate(s, "rra\n"));
	}
	is_sorted(s);
	free_stack(&h);
